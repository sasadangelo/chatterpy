...
class QdrantDatabase(Database):
    def __init__(self, config, embeddings):
        self.config = config
        self.qdrant_path = os.path.expanduser(
            self.config.get("qdrant_path", DEFAULT_QDRANT_PATH)
        )
        self.qdrant_collection = self.config.get(
        ...
        if not os.path.exists(self.qdrant_path):
            ...
            raise NotADirectoryError(
                f"{self.qdrant_path} exists but is not a directory."
            )
        else:
        ...
        try:
            ...
            print(
                f"Qdrant collection {self.qdrant_collection} does not exist. Creating the collection..."
            )
    ...
    def get_context(self, user_message):
        ...
        if self.qdrant:
            context = [
                c.page_content
                for c in self.qdrant.similarity_search(user_message, k=rag_top_k_chunks)
            ]
        return context
